...
class Perbaikan extends MY_Controller {
    ...
    function __construct(){
        parent::__construct();
        $this->load->model('PerbaikanModel');
    ...
    function ubahkondisi($intid){
        ...
        $this->load->view($this->view . '/ubahkondisi',$data);
    }
    ...
    function simpanroom($intperbaikan, $intpallet, $introom){
        ...
        $result = $this->modelapp->updatedata($this->table,$data,$intperbaikan);
        if ($result) {
            redirect(base_url($this->controller . '/view'));
        }
    }
    ...
    function exportexcel(){
        ...
        $excel->getProperties()->setCreator('')
                     ->setLastModifiedBy('')
                     ->setTitle("Report Pallet Repair " . $judul)
                     ->setSubject("Report Pallet Repair")
                     ->setDescription("Report Pallet Repair")
                     ->setKeywords("Report Pallet Repair");
        ...
        foreach ($datagedung as $gedung) {
            ...
            $excel->setActiveSheetIndex($loop)->setCellValue('B2', "Report Pallet Repair, on Date : ". date('d-m-Y',strtotime($from)) . " To ". date('d-m-Y',strtotime($to)));
            ...
            $excel->setActiveSheetIndex($loop)->setCellValue('J3', "Current Condition");
        ...
        header('Content-Disposition: attachment; filename="Report Pallet Repair ' .$judul. '.xls"'); // Set nama file excel nya
