...
class Pinjam extends CI_Controller {
    ...
    function __construct(){
        ...
        $this->load->model('Production/PinjamModel');
    ...
    function index(){
        redirect(base_url('production/pinjam/view'));
    }
    ...
    function view($halaman=1){
        ...
        $jmldata     = $this->model->getjmldata('pr_pinjam', $date1, $date2, $intgedung, $intcell, $intkaryawan);
        $offset      = ($halaman - 1) * 10;
        $jmlpage     = ceil($jmldata[0]->jmldata / 10);

        $data['title']        = $datagedung[0]->vcnama . ' - Borrow';
        $data['controller']   = 'production/pinjam';
        $data['from']         = $from;
        $data['to']           = $to;
        ...
        $data['dataP']        = $this->model->getdatalimit('pr_pinjam',$offset,10, $date1, $date2, $intgedung, $intcell, $intkaryawan);
        
        $this->template->set_layout('default_prod')->build('production/pinjam_view/index',$data);
    }

    function detail($intid){
        $data['controller'] = 'production/pinjam';
        $data['dataMain']   = $this->model->getdatadetail('pr_pinjam',$intid);
        $data['datapallet'] = $this->model->getdatapinjampallet($intid);
        $this->load->view('production/pinjam_view/detail',$data);
    }

    function edit($intid){
        $resultData = $this->model->getdatadetail('pr_pinjam',$intid);
        $data = array(
        ...
        $this->template->set_layout('default_prod')->build('production/pinjam_view/form',$data);
    }
    ...
    function validasiform($tipe){
        ...
        if ($tipe == 'data') {
            foreach ($data as $key => $value) {
                $result = $this->modelapp->getdatadetailcustom('pr_pinjam',$value,$key);
    ...
    function aksi($tipe,$intid,$status=0){
        if ($tipe == 'Add') {
            ...
            $result = $this->modelapp->insertdata('pr_pinjam',$data);
            if ($result) {
                redirect(base_url('production/pinjam' . '/view'));
            }
        } elseif ($tipe == 'Edit') {
            ...
            $result = $this->modelapp->updatedata('pr_pinjam',$data,$intid);
            if ($result) {
                redirect(base_url('production/pinjam' . '/view'));
            }
        } elseif ($tipe == 'Delete') {
    ...
    function add(){
        ...
        $this->template->set_layout('default_prod')->build('production/pinjam_view/form_add',$data);
    }
    ...
    function simpan_peminjam() {
        ...
        if ($result) {
            ...
            redirect(base_url('production/pinjam/transaksi/' . $datapinjam[0]->intid));
        }
    }
