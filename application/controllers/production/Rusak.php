...
class Rusak extends CI_Controller {
    ...
    function __construct(){
        ...
        $this->load->model('production/RusakModel');
    ...
    function index(){
        redirect(base_url('production/rusak' . '/view'));
    }
    ...
    function view($halaman=1){
        ...
        $intproses  = ($this->input->get('intproses') == '') ? 0 : $this->input->get('intproses');$intproses = ($this->input->get('intproses') == '') ? 0 : $this->input->get('intproses');
        ...
        $jmldata    = $this->model->getjmldata('pr_rusak', $date1, $date2, $intgedung, $intmodel, $intproses);
        $offset     = ($halaman - 1) * 10;
        $jmlpage    = ceil($jmldata[0]->jmldata / 10);

        $data['title']      = $datagedung[0]->vcnama . ' - Destroy';
        $data['controller'] = 'production/rusak';
        $data['from']       = $from;
        $data['to']         = $to;
        ...
        $data['dataP']      = $this->model->getdatalimit('pr_rusak',$offset,10, $date1, $date2, $intgedung, $intmodel, $intproses);
        
        $this->template->set_layout('default_prod')->build('production/rusak_view' . '/index',$data);
    }

    function detail($intid){
        $data['controller']  = 'production/rusak';
        $data['dataMain']    = $this->model->getdatadetail('pr_rusak',$intid);
        $data['dataHistory'] = $this->modelapp->getdatahistory($this->tablehistory,$intid);
        $this->load->view('production/rusak_view' . '/detail',$data);
    }
    ...
    function add(){
        ...
        $this->template->set_layout('default_prod')->build('production/rusak_view' . '/form',$data);
    }

    function edit($intid){
        $resultData = $this->model->getdatadetail('pr_rusak',$intid);
        $data = array(
    ...
    function validasiform($tipe){
        ...
        if ($tipe == 'data') {
            foreach ($data as $key => $value) {
                $result = $this->modelapp->getdatadetailcustom('pr_rusak',$value,$key);
    ...
    function aksi($tipe,$intid,$status=0){
        if ($tipe == 'Add') {
            ...
            $result = $this->modelapp->insertdata('pr_rusak',$data);
            if ($result) {
                redirect(base_url('production/rusak' . '/view'));
            }
        } elseif ($tipe == 'Edit') {
            ...
            $result = $this->modelapp->updatedata('pr_rusak',$data,$intid);
            if ($result) {
                redirect(base_url('production/rusak' . '/view'));
            }
        } elseif ($tipe == 'Delete') {
    ...
    function ubahkondisi($intid){
        $datapallet         = $this->model->cekdatapallet($intid);
        $data['controller'] = 'production/rusak';
        $data['intid']      = $intid;
        $data['intpallet']  = $datapallet[0]->intpallet;

        $this->load->view('production/rusak_view' . '/ubahkondisi',$data);
    }

    function kondisiubah($intpallet, $intid, $intkondisi){
        if ($intkondisi == 1) {
            $datapallet    = $this->modelapp->getdatadetail('m_pallet',$intpallet);
            $intqty        = $datapallet[0]->intqty;
            $intstoktambah = $intqty + 1;
            $datatambah = array(
                'intqty' => $intstoktambah
            );
            $this->modelapp->updatedata('m_pallet',$datatambah,$intpallet);
