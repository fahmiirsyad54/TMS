...
class RoomModel extends CI_Model {
    ...
    function getdatalimit($table,$halaman=0, $limit=5, $keyword=''){
        $this->db->select('a.intid, a.vckode, a.vcnama, a.intstatus, ISNULL(c.vcnama, 0) as vcrak, ISNULL(d.vcnama, 0) as vcgedung,
                            ISNULL(b.vcnama, 0) as vcstatus, ISNULL(b.vcwarna, 0) as vcstatuswarna ',false);
         $this->db->from($table . ' as a');
         $this->db->join('app_mstatus' . ' as b', 'a.intstatus = b.intstatus', 'left');
         $this->db->join('m_rak as c', 'c.intid = a.intrak', 'left');
         $this->db->join('m_gedung as d', 'd.intid = c.intgedung', 'left');
         $this->db->like('a.vcnama', $keyword);
         $this->db->or_like('a.vckode', $keyword);
    ...
    public function buat_kode()   {
          $this->db->select('RIGHT(m_room.vckode,5) as kode', FALSE);
    ...
    public function buat_nama($intrak)   {
        $this->db->select('RIGHT(m_room.vcnama,2) as kode', FALSE);
        $this->db->where('intrak',$intrak);
        $this->db->order_by('intid','DESC');    
        $this->db->limit(1);    
        $query = $this->db->get('m_room');      //cek dulu apakah ada sudah ada kode di tabel.    
        if($query->num_rows() <> 0){      
        ...
        $kodejadi = "R".$kodemax;    // hasilnya ODJ-9921-0001 dst.
        return $kodejadi;  
    }
