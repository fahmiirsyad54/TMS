...
class RusakModel extends CI_Model {
    ...
    function getjmldata($table, $from=null, $to=null, $intgedung=0, $intmodel=0, $intproses=0){
        ...
        if ($from) {
            $this->db->where('a.dtrusak >= ', $from);
            $this->db->where('a.dtrusak <= ', $to);
        }
        if ($intmodel > 0) {
    ...
    function getdata($from=null, $to=null, $intgedung=0, $intmodel=0, $intproses=0){
        $this->db->select('a.intid, a.dtrusak, ISNULL(c.vckode, 0) as vckode, ISNULL(d.vcnama, 0) as vcmodel, 
                        ISNULL(e.vcnama, 0) as vcproses, ISNULL(f.vcnama, 0) as vcsize, ISNULL(g.vcnama, 0) as vcside, 
                        ISNULL(i.vcnama, 0) as vcgedung, ISNULL(l.vcnama, 0) as vcuser',false);
    ...
    function getdatalimit($table,$halaman=0, $limit=5, $from=null, $to=null, $intgedung=0, $intmodel=0, $intproses=0){
        $this->db->select('a.intid, a.intpallet, a.intstatus, ISNULL(b.vcnama, 0) as vcstatus, ISNULL(b.vcwarna, 0) as vcstatuswarna,
                            ISNULL(c.vcnama, 0) as vcgedung, ISNULL(d.vcnama, 0) as vcmodel, ISNULL(e.vcnama, 0) as vcproses, ISNULL(f.vckode, 0) as vckode,
                            ISNULL(g.vcnama, 0) as vcuser, convert(varchar, a.dtrusak, 6) as dtrusak,',false);
        $this->db->from($table . ' as a');
        $this->db->join('m_pallet as f','a.intpallet = f.intid','left');
        $this->db->join('app_mstatus' . ' as b', 'a.intstatus = b.intid', 'left');
    ...
    function cekdatapallet($intid){
        $this->db->select('a.intid, a.intpallet',false);
