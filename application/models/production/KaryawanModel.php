...
class KaryawanModel extends CI_Model {
    ...
    function getjmldata($table, $keyword='', $intgedung){
    ...
    function getdatalimit($table,$halaman=0, $limit=5, $keyword='', $intgedung){
        $this->db->select('a.intid, a.intgedung, a.vckode, a.vcnama, a.intstatus, ISNULL(c.vcnama, 0) as vcjabatan, ISNULL(d.vcnama, 0) as vcgedung, ISNULL(b.vcnama, 0) as vcstatus, ISNULL(b.vcwarna, 0) as vcstatuswarna',false);
        $this->db->from($table . ' as a');
        $this->db->join('app_mstatus' . ' as b', 'a.intstatus = b.intstatus', 'left');
        $this->db->join('m_jabatan' . ' as c', 'a.intjabatan = c.intid', 'left');
        $this->db->join('m_gedung' . ' as d', 'a.intgedung = d.intid', 'left');
